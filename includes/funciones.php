...
function buscar_pago_creado($idsocio)
{
	global $mysqli;
	$retorno = array();
	$idsocio = escapar($idsocio);
	$c = "SELECT * from socios_pagos WHERE estado = 0 and idsocio = $idsocio ";
	$r = $mysqli->query($c);
	if ($r->num_rows > 0) {
...
function crear_nuevo_pago($idsocio)
{
	global $mysqli;
	$retorno = array();
	$idsocio = escapar($idsocio);
	$mysqli->query("UPDATE socios_pagos SET estado = -1 WHERE estado = 0 AND idsocio = $idsocio ");
	$c = "INSERT INTO socios_pagos (idsocio) VALUES ($idsocio); ";
	if ($mysqli->query($c)) {
		$retorno = $mysqli->insert_id;
	}
	return $retorno;
}

function buscar_facturas_pendientes($idpago)
{
	global $mysqli;
	$retorno = array();
	$idpago = escapar($idpago);
	$c = "SELECT * from factura_update WHERE nropago = $idpago ";
	$r = $mysqli->query($c);
	while ($f = $r->fetch_assoc()) {
		$retorno[] = $f;
	}
...
function buscar_factura_pendiente($idfactura)
{
	global $mysqli;
	$retorno = array();
	$idfactura = escapar($idfactura);
	$c = "SELECT * from factura_update WHERE idfactura = $idfactura ";
	$r = $mysqli->query($c);
	if ($r->num_rows > 0) {
		$retorno = $r->fetch_assoc();
	}
...
function crear_factura_pendiente($idfactura, $nropago = 0)
{
	global $mysqli;
	$retorno = false;
	$idfactura = escapar($idfactura);
	$nropago = escapar($nropago);
	$c = "SELECT * from factura_update WHERE idfactura = $idfactura ";
	$r = $mysqli->query($c);
	if ($r->num_rows === 0) {
		if ($mysqli->query("INSERT INTO factura_update (`IDFACTURA`, 
														`IDSOCIO`, 
														`IDUSUARIO`, 
														`MONTOCOBERTURA`, 
														`INTERES`, 
...
function actualizar_factura_pendiente($idfactura, $nropago = 0)
{
	global $mysqli;
	$retorno = false;
	$idfactura = escapar($idfactura);
	$nropago = escapar($nropago);
	$mysqli->query("UPDATE factura_update SET NROPAGO = '$nropago' WHERE IDFACTURA = $idfactura");
	return $retorno;
}

function registrar_pago_finalizado($nropago)
{
	global $mysqli;
	$retorno = false;
	$nropago = escapar($nropago);
	$retorno = $mysqli->query("UPDATE factura_update SET estado = 1, FECHA_PAGO = '" . date('Y-m-d') . "', HORA_PAGO = '" . date('H:i:s') . "' WHERE NROPAGO = $nropago");
	$c = "SELECT * from factura_update WHERE nropago = $nropago ";
	$r = $mysqli->query($c);
	if ($r->num_rows > 0) {
		while ($f = $r->fetch_assoc()) {
			$mysqli->query("DELETE FROM factura WHERE idfactura = '" . $f['IDFACTURA'] . "'");
		}
		$mysqli->query("UPDATE socios_pagos SET estado = 2 WHERE id = '$nropago' ");
		$retorno = true;
	}
	return $retorno;
}

function registrar_pago_pendiente($nropago)
{
	global $mysqli;
	$retorno = false;
	$nropago = escapar($nropago);
	$retorno = $mysqli->query("UPDATE factura_update SET estado = 2, FECHA_PAGO = '" . date('Y-m-d') . "', HORA_PAGO = '" . date('H:i:s') . "' WHERE NROPAGO = $nropago");
	$c = "SELECT * from factura_update WHERE nropago = $nropago ";
	$r = $mysqli->query($c);
	if ($r->num_rows > 0) {
		while ($f = $r->fetch_assoc()) {
			echo "DELETE FROM factura WHERE idfactura = '" . $f['IDFACTURA'] . "'";
		}
	}
	return $retorno;
}

function buscar_pago_finalizado($nropago)
{
	global $mysqli;
	$retorno = [];
	$nropago = escapar($nropago);
	$c = "SELECT * from factura_update WHERE nropago = $nropago and estado = 1 ";
	$r = $mysqli->query($c);
	if ($r->num_rows > 0) {
		while ($f = $r->fetch_assoc()) {
			$retorno[] = $f;
...
function buscar_pagos_finalizados($idsocio)
{
	global $mysqli;
	$retorno = [];
	$idsocio = escapar($idsocio);
	$c = "SELECT DISTINCT(nropago) from factura_update WHERE IDSOCIO = $idsocio and estado = 1 ";
	$r = $mysqli->query($c);
	if ($r->num_rows > 0) {
		while ($f = $r->fetch_assoc()) {
			$retorno[] = $f;
...
function obtener_nro_pago_de_transaccion($transaccionId)
{
	return intval(str_replace(PREFIJO_PAGOS . '-', '', $transaccionId));
}

function formatear_fecha($fecha, $formato = 'd/m/Y', $extra = '')
{
	return date($formato, strtotime($fecha . ' ' . $extra));
}

function formatear_numero($n, $d = 2)
{
	return number_format($n, $d, ",", ".");
}

function buscar_socio($idsocio)
{
	global $mysqli;
	$retorno = [];
	$idsocio = escapar($idsocio);
	$c = "SELECT * from socios WHERE IDSOCIO = $idsocio ";
	$r = $mysqli->query($c);
	if ($r->num_rows > 0) {
