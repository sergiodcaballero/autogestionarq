...
<table class="tabla c-12">
    ...
    <tbody>
        ...
        foreach ($tramites as $tramite) {
            ...
            $r = $mysqli->query("SELECT * FROM tramites_detalle WHERE tra_id='{$tramite['id']}' AND fc_id = 1"); //OBTENGO EL APELLIDO;

            $f = $r->fetch_assoc();

            $apellido = $f['valor'];

            $r = $mysqli->query("SELECT * FROM tramites_detalle WHERE tra_id='{$tramite['id']}' AND fc_id = 2"); //OBTENGO EL NOMBRE;

            $f = $r->fetch_assoc();

            $nombre = $f['valor'];

            $r = $mysqli->query("SELECT * FROM tramites_detalle WHERE tra_id='{$tramite['id']}' AND fc_id = 20"); //OBTENGO EL CORREO;
